...
//
// NOTE (2026-09-24): matmul kernels are deliberately NAIVE (one thread per
// output, bounds-checked early exit). A 16x16-tiled variant caused error
// 700 faults and a machine lockup on RTX 3050 and was reverted; the naive
// form is ~800x CPU at 128^3 regardless. Tiling stays on OpenCL/Metal,
// whose index arithmetic differs. Do NOT re-tile without per-size native
// verification (diag harness + timeouts) first.
...
// fallback) instead of blocking forever on a wedged context. Allocation
// failures (transient OOM) do NOT poison.
static void cuda_poison(void) {
    cuda_state = -1;
}
...
static int32_t cuda_launch_ew(void *ah, void *bh, void *oh, int32_t count, int op, int32_t dtype) {
    ...
    {
        int rc = p_cuLaunchKernel((CUfunction)cuda_fns[ki], blocks, 1, 1, 256, 1, 1, 0, 0, args, 0);
        if (rc != CUDA_SUCCESS) {
            snprintf(cuda_error, sizeof(cuda_error), "ew launch failed: %d", rc);
            cuda_poison();
            return 0;
        }
        rc = p_cuCtxSynchronize();
        if (rc != CUDA_SUCCESS) {
            snprintf(cuda_error, sizeof(cuda_error), "ew sync failed: %d", rc);
            cuda_poison();
            return 0;
        }
    }
...
int32_t alya_tensor_cuda_matmul(void *ah, void *bh, void *oh, int32_t a_off, int32_t b_off, int32_t r_off,
    ...
    args[7] = &nn;
    args[8] = &kk;
    // Tiled kernels run in fixed 16x16 work-groups: grid counts blocks
    // (bounds checks cover partial edge tiles), local size is pinned.
    gx = (unsigned int)(((int64_t)n + 15) / 16);
    gy = (unsigned int)(((int64_t)m + 15) / 16);
    // CUDA caps y/z grid dims at 65535: decline oversized launches so the
    // caller falls back instead of feeding the driver invalid dimensions.
    if (gx == 0 || gy == 0 || gx > 65535 || gy > 65535) return 0;
    {
        int rc = p_cuLaunchKernel((CUfunction)cuda_fns[ki], gx, gy, 1, 16, 16, 1, 0, 0, args, 0);
        if (rc != CUDA_SUCCESS) {
            snprintf(cuda_error, sizeof(cuda_error), "mm launch failed: %d", rc);
            cuda_poison();
            return 0;
        }
        rc = p_cuCtxSynchronize();
        if (rc != CUDA_SUCCESS) {
            snprintf(cuda_error, sizeof(cuda_error), "mm sync failed: %d", rc);
            cuda_poison();
            return 0;
        }
    }

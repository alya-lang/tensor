...
static const char *cuda_ptx_src =
    ...
    "{\n"
    "    .reg .u64 %rdA, %rdB, %rdC, %off, %rdS;\n"
    "    .reg .u32 %i, %j, %li, %lj, %p, %M, %N, %K, %ao, %bo, %ro, %t, %cx, %cy, %q;\n"
    "    .reg .pred %pM, %pN, %pK, %pA, %pB;\n"
    "    .reg .f32 %s, %x, %y;\n"
    ...
    "    mov.u32 %lj, %tid.x;\n"
    "    mov.u32 %cy, %ctaid.y;\n"
    "    mad.lo.u32 %i, %cy, %t, %li;\n"
    "    mov.u32 %t, %ntid.x;\n"
    "    mov.u32 %cx, %ctaid.x;\n"
    "    mad.lo.u32 %j, %cx, %t, %lj;\n"
    "    ld.param.u32 %M, [M];\n"
    "    ld.param.u32 %N, [N];\n"
    "    ld.param.u32 %K, [K];\n"
    "    ld.param.u32 %ao, [ao];\n"
    ...
    "    @%pA ld.global.f32 %x, [%off];\n"
    "    mad.lo.u32 %t, %li, 16, %lj;\n"
    "    cvt.u64.u32 %off, %t;\n"
    "    shl.b64 %off, %off, 2;\n"
    "    mov.u64 %rdS, As;\n"
    "    add.u64 %off, %rdS, %off;\n"
    ...
    "    @%pB ld.global.f32 %y, [%off];\n"
    "    mad.lo.u32 %t, %cy, 16, %lj;\n"
    "    cvt.u64.u32 %off, %t;\n"
    "    shl.b64 %off, %off, 2;\n"
    "    mov.u64 %rdS, Bs;\n"
    "    add.u64 %off, %rdS, %off;\n"
    ...
    "    setp.ge.u32 %pK, %q, 16;\n"
    "    @%pK bra $next;\n"
    "    mad.lo.u32 %t, %li, 16, %q;\n"
    "    cvt.u64.u32 %off, %t;\n"
    "    shl.b64 %off, %off, 2;\n"
    "    mov.u64 %rdS, As;\n"
    "    add.u64 %off, %rdS, %off;\n"
    "    ld.shared.f32 %x, [%off];\n"
    "    mad.lo.u32 %t, %q, 16, %lj;\n"
    "    cvt.u64.u32 %off, %t;\n"
    "    shl.b64 %off, %off, 2;\n"
    "    mov.u64 %rdS, Bs;\n"
    "    add.u64 %off, %rdS, %off;\n"
    ...
    "    .reg .pred %pM, %pN, %pK, %pA, %pB;\n"
    "    .reg .f64 %s, %x, %y;\n"
    "    .shared .align 8 .b8 As[2048];\n"
    "    .shared .align 8 .b8 Bs[2048];\n"
    "    mov.u32 %li, %tid.y;\n"
    "    mov.u32 %t, %ntid.y;\n"
    "    mov.u32 %lj, %tid.x;\n"
    "    mov.u32 %cy, %ctaid.y;\n"
    ...
    "    add.u64 %off, %rdA, %off;\n"
    "    mov.f64 %x, 0D0000000000000000;\n"
    "    @%pA ld.global.f64 %x, [%off];\n"
    "    mad.lo.u32 %t, %li, 16, %lj;\n"
    "    cvt.u64.u32 %off, %t;\n"
    "    shl.b64 %off, %off, 3;\n"
    "    mov.u64 %rdS, As;\n"

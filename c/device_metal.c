...
static const char *metal_kernel_src =
    ...
    "    int ao = P[0]; int bo = P[1]; int ro = P[2];\n"
    "    int M = P[3]; int N = P[4]; int K = P[5];\n"
    "    threadgroup int As[16][16];\n"
    "    threadgroup int Bs[16][16];\n"
    "    int s = 0;\n"
    "    int tiles = (K + 15) / 16;\n"
    "    for (int t = 0; t < tiles; ++t) {\n"
    "        int pa = t * 16 + lj;\n"
    ...
    "    int ao = P[0]; int bo = P[1]; int ro = P[2];\n"
    "    int M = P[3]; int N = P[4]; int K = P[5];\n"
    "    threadgroup long As[16][16];\n"
    "    threadgroup long Bs[16][16];\n"
    "    long s = 0;\n"
    "    int tiles = (K + 15) / 16;\n"
    "    for (int t = 0; t < tiles; ++t) {\n"
    "        int pa = t * 16 + lj;\n"
...
int32_t alya_tensor_metal_matmul(void *ah, void *bh, void *oh, int32_t a_off, int32_t b_off, int32_t r_off,
    ...
    params[4] = n;
    params[5] = k;
    // Tiled kernels need full 16x16 groups: round the grid up, bounds
    // checks cover partial edge tiles.
    if (!metal_run(metal_pipes[ki], (struct metal_buffer *)ah, (struct metal_buffer *)bh,
                   (struct metal_buffer *)oh, params, 6,
                   (unsigned long)(((int64_t)n + 15) / 16 * 16),
                   (unsigned long)(((int64_t)m + 15) / 16 * 16))) {
        return 0;
    }
